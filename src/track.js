...
export const TRACKS = [
  { name: '🌿 草原赛道', fogColor: 0x87ceeb },
  { name: '🏜️ 沙漠峡谷', fogColor: 0xd4a855 },
  { name: '❄️ 雪山赛道', fogColor: 0xc8e8ff },
];
...
function _genGrass() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    pts.push(new THREE.Vector3(Math.cos(t) * 80, y, Math.sin(t) * 50));
  }
  return pts;
}
...
function _genDesert() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    pts.push(new THREE.Vector3(
      Math.cos(t) * 65 + Math.sin(t * 2) * 15,
      0,
      Math.sin(t) * 38 + Math.cos(t * 2) * 8
    ));
  }
  return pts;
}
...
function _genSnow() {
  ...
  for (let i = 0; i < N; i++) {
    const t = (i / N) * Math.PI * 2;
    pts.push(new THREE.Vector3(
      Math.cos(t) * 95 + Math.sin(t * 2) * 18,
      Math.max(0, Math.sin(t * 2) * 10 + Math.sin(t * 3) * 4),
      Math.sin(t) * 62
    ));
  }
  return pts;
...
const _gens = [_genGrass, _genDesert, _genSnow];

export function generateTrackPoints(trackIdx = 0) {
  return _gens[trackIdx]();
}
...
export function buildTrack(scene, world, trackIdx = 0) {
  const trackPoints = generateTrackPoints(trackIdx);
  ...
  _buildGround(group, world, trackIdx);
  _buildStartArch(trackPoints, trackWidth, group);
  _buildDecorations(trackPoints, trackWidth, group, trackIdx);
...
function _buildGround(group, world, theme) {
  ...
  for (let y = 0; y < 256; y += 4)
    for (let x = 0; x < 256; x += 4) {
      ...
      ctx.fillStyle = theme === 1
        ? `rgb(${g|0},${g*.75|0},${g*.3|0})`
        : theme === 2
        ? `rgb(${g*.9|0},${g*.95|0},${g|0})`
        : `rgb(${g*.6|0},${g|0},${g*.4|0})`;
...
function _buildStartArch(trackPoints, trackWidth, group) {
  ...
  banner.rotation.y = Math.atan2(-dir.z, dir.x);
  group.add(banner);
}

function _buildDecorations(trackPoints, trackWidth, group, theme) {
  ...
  for (let i = 0; i < trackPoints.length; i += 2) {
    ...
    for (const side of [-1, 1]) {
      ...
      if (theme === 1) {
        ...
        // Snow: trees + rocks
        if (i % 10 === 0) {
          const tree = createVoxelTree(3 + Math.floor(Math.random() * 3));
